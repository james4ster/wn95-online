...
function buildBracket(playoffGames, numTeams) {
    ...
    const addTo = (key) => {
      ...
      if (
        !rMap
          ...
          .find(
            (x) =>
              x.game_number === g.game_number && x.team_code_a === g.team_code_a
          )
      ) {
        rMap.get(key).push(g);
      }
  ...
  byRound.forEach((rMap) => {
    rMap.forEach((games, k) => {
      rMap.set(
        k,
        games.sort((a, b) => (a.game_number ?? 0) - (b.game_number ?? 0))
      );
    });
  });
  ...
  function getSeriesLength(games) {
    for (const g of games) {
      if (g.series_length != null && g.series_length > 0)
        return g.series_length;
    }
    return 7;
  }
...
function getRsRec(t1, t2, sg) {
  ...
  const gs = sg.filter(
    (g) => (g.home === t1 && g.away === t2) || (g.home === t2 && g.away === t1)
  );
  ...
  return (
    <div
      style={{
        ...
        minHeight: 28,
        background: isWinner
          ? 'linear-gradient(90deg,rgba(0,210,90,.15),rgba(0,210,90,.03))'
          : 'transparent',
        borderLeft:
          !flipped && isWinner ? '3px solid #00FF88' : '3px solid transparent',
        borderRight:
          flipped && isWinner ? '3px solid #00FF88' : '3px solid transparent',
              ...
              background:
                i < wins
                  ? isWinner
                    ...
                    : 'linear-gradient(135deg,#87CEEB,#4a9ec4)'
                  : 'rgba(255,255,255,.07)',
              boxShadow:
                i < wins
                  ? isWinner
                    ? '0 0 8px rgba(255,215,0,.9)'
                    : '0 0 6px rgba(135,206,235,.7)'
                  : 'none',
            }}
          />
        ))}
      ...
      {[topTeam, botTeam].map((team, idx) => (
        ...
        >
          ...
          {gCapped.map((g, i) => {
            const aT = idx === 0;
            const ts =
              g.team_code_a === topTeam ? g.team_a_score : g.team_b_score;
            const bs =
              g.team_code_a === topTeam ? g.team_b_score : g.team_a_score;
            const val = aT ? ts : bs;
    ...
    return (
      <div
        style={{
          ...
          letterSpacing: 1,
          textShadow: '0 0 10px rgba(255,215,0,.8)',
        }}
      >
        {winner} WINS {winnerW}-{loserW}
      </div>
    );
  }
    ...
    <div>
      {hasGames &&
        (() => {
          let label, color;
          if (topW > botW) {
            label = `${topTeam} LEADS ${topW}-${botW}`;
            color = '#00FF88';
          } else if (botW > topW) {
            label = `${botTeam} LEADS ${botW}-${topW}`;
            color = '#00FF88';
          } else {
            label = `TIED ${topW}-${botW}`;
            color = '#87CEEB';
          }
          return (
            <div
      ...
      {rsRec &&
        (() => {
          ...
          const text = tied
            ? `RS TIED ${w1}-${w2}`
            : `${w1 > w2 ? topTeam : botTeam} LED RS ${Math.max(
                w1,
                w2
              )}-${Math.min(w1, w2)}`;
          return (
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 9,
        background:
          'linear-gradient(155deg,rgba(8,8,22,.98),rgba(16,16,38,.98))',
    ...
    >
      <div
        style={{
          ...
          background: `radial-gradient(circle at top right,${
            done ? 'rgba(255,215,0,.08)' : 'rgba(135,206,235,.04)'
          },transparent 70%)`,
          pointerEvents: 'none',
        }}
      />
      ...
      <div
        style={{
          height: 1,
          background:
            'linear-gradient(90deg,transparent,rgba(255,140,0,.18),transparent)',
        }}
      />
      {games.length > 0 ? (
        <ScoreGrid
    ...
    return (
      <div
        style={{
          ...
          minHeight: 42,
          background: isWinner
            ? 'linear-gradient(90deg,rgba(0,210,90,.18),rgba(0,210,90,.04))'
            : 'transparent',
            ...
            <div
              ...
              style={{
                ...
                boxShadow:
                  i < wins
                    ? isWinner
                      ? '0 0 9px rgba(255,215,0,.9)'
                      : '0 0 7px rgba(135,206,235,.7)'
                    : 'none',
              }}
            />
          ))}
    ...
    >
      <div style={{ textAlign: 'center' }}>
        {!tErr ? (
          <img
            src={tSrc}
            alt="Trophy"
            onError={() => setTErr(true)}
            style={{
              width: 52,
              height: 52,
              filter: 'drop-shadow(0 0 18px rgba(255,215,0,.92))',
            }}
          />
        ) : (
          <div
      ...
      <div
        ...
        style={{
          ...
          borderRadius: 10,
          background:
            'linear-gradient(155deg,rgba(14,11,30,.99),rgba(30,22,6,.99))',
          boxShadow:
            '0 0 28px rgba(255,215,0,.26),0 0 56px rgba(255,140,0,.12),inset 0 0 28px rgba(255,215,0,.07)',
          overflow: 'hidden',
          position: 'relative',
        }}
      >
        <div
          style={{
            ...
            pointerEvents: 'none',
            background:
              'linear-gradient(45deg,transparent 30%,rgba(255,215,0,.04) 50%,transparent 70%)',
        ...
        <div
          style={{
            height: 1,
            background:
              'linear-gradient(90deg,transparent,rgba(255,215,0,.4),transparent)',
            margin: '2px 0',
          }}
        />
        ...
        <div
          style={{
            height: 1,
            background:
              'linear-gradient(90deg,transparent,rgba(255,140,0,.2),transparent)',
          }}
        />
        {(games || []).length > 0 ? (
          <ScoreGrid
...
export default function PlayoffBracket({
  ...
  const numTeams =
    ...
    (() => {
      ...
      const teams = new Set(
        playoffGames.flatMap((g) =>
          [g.team_code_a, g.team_code_b].filter(Boolean)
        )
      );
      return Math.pow(2, Math.ceil(Math.log2(Math.max(teams.size, 2))));
    })();

  const { leftRounds, rightRounds, champSlot, nRounds } = buildBracket(
    playoffGames,
    numTeams
  );
  const CARD_W = 192,
  ...
  leftRounds.forEach((rnd, ri) => {
    ...
    rnd.forEach((_, mi) => {
      if (!leftRefs.current[ri][mi])
        leftRefs.current[ri][mi] = React.createRef();
    });
  });
  rightRounds.forEach((rnd, ri) => {
    if (!rightRefs.current[ri]) rightRefs.current[ri] = [];
    rnd.forEach((_, mi) => {
      if (!rightRefs.current[ri][mi])
        rightRefs.current[ri][mi] = React.createRef();
    });
  });
  ...
  const leftCols = leftRounds.map((_, ri) =>
    col(leftRounds, leftRefs.current, ri, false)
  );
  const rightCols = rightRounds
    .map((_, ri) => col(rightRounds, rightRefs.current, ri, true))
    .reverse();
