...
// ── Layout configs — all pixel constants live here ─────────────────────────
const LAYOUTS = {
  '16:9': {
    rootW: 1280, rootH: 720,
    topbarH: 44, scrollerH: 36,
    sidePanelW: 208,
    heroLogoSize: 74,
    heroWinsFontSize: '5.6rem',
    heroSeedFontSize: '1rem',
    sidePanelLogoSize: 28,
    sideCodeFontSize: '.86rem',
    leagueLogoH: 38,
    emptyLogoH: 80,
  },
  '4:3': {
    ...
    sidePanelLogoSize: 26,
    sideCodeFontSize: '.78rem',
    leagueLogoH: 36,
    emptyLogoH: 80,
  },
};
...
const getParams = () => {
  ...
  return {
    ...
    format: p.get('format') || '16:9',
  };
};

// ── Main Component ─────────────────────────────────────────────────────────
// format prop: '16:9' (default) | '4:3'
export default function StreamOverlayPlayoff() {
  const [pendingFormat, setPendingFormat] = useState(getParams().format);
  const layout = LAYOUTS[pendingFormat] ?? LAYOUTS['16:9'];
  ...
  const scale = useOverlayScale(layout.rootW, layout.rootH);
  ...
  const loadMatchup = useCallback(async (tA, tB, lg, round, series) => {
    ...
    const h2h = {
      ...
      seasonAW: sAW, seasonBW: sBW, seasonTies: sTies,
      ...
      allTimeAW: atAW, allTimeBW: atBW, allTimeTies: atTies,
  ...
  const handleApply = useCallback(() => {
    ...
    const p = new URLSearchParams({ lg: currentLg, teamA: pendingA, teamB: pendingB, format: pendingFormat });
    window.history.replaceState(null, '', `?${p.toString()}`);
    setShowPanel(false);
    loadMatchup(pendingA, pendingB, currentLg, null, null);
  }, [pendingA, pendingB, currentLg, pendingFormat, loadMatchup]);
  ...
  return (
    ...
    >
     {showPanel && (
        <SetupPanel allTeams={allTeams} pendingA={pendingA} setPendingA={setPendingA}
          pendingB={pendingB} setPendingB={setPendingB}
          pendingFormat={pendingFormat} setPendingFormat={setPendingFormat}
          onApply={handleApply} />
      )}
      {!data && !loading && <EmptyState layout={layout} />}
      {loading && !data && <LoadingState />}
      {data && <PlayoffLayout data={data} loading={loading} layout={layout} />}
...
// ── Playoff Layout ─────────────────────────────────────────────────────────
function PlayoffLayout({ data, loading, layout }) {
  const {
  ...
  return (
    <>
      ...
      <SeriesHero leftTeam={leftTeam} rightTeam={rightTeam}
        ...
        winsNeeded={winsNeeded} layout={layout} />

      <div className="po-side-left">
        <SidePanel team={leftTeam} skaters={leftSkaters} teamStats={leftStats} layout={layout} />
      </div>
      <div className="po-side-right">
        <SidePanel team={rightTeam} skaters={rightSkaters} teamStats={rightStats} layout={layout} />
      </div>
...
// ── Series Hero Bar ────────────────────────────────────────────────────────
function SeriesHero({ leftTeam, rightTeam, leftWins, rightWins, leftSeed, rightSeed, winsNeeded, layout }) {
  return (
...
function SidePanel({ team, skaters, teamStats, layout }) {
...
// ── Setup Panel ────────────────────────────────────────────────────────────
function SetupPanel({ allTeams, pendingA, setPendingA, pendingB, setPendingB, pendingFormat, setPendingFormat, onApply }) {
  return (
    <div className="sp-panel">
      <div className="sp-title">⚡ PLAYOFF OVERLAY SETUP</div>
      <div className="sp-note">Press ~ to close</div>
      <div className="sp-row">
        <label className="sp-label">SCREEN RATIO</label>
        <CustomSelect
          value={pendingFormat}
...
// ── Styles — layout-aware via CSS custom properties ────────────────────────
function Styles({ layout }) {
  ...
  return (
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .po-root {
      width: ${v.rootW}px; height: ${v.rootH}px;
    ...
    /* ── TOP BAR ── */
    .po-topbar {
      position: absolute; top: 0; left: 0; right: 0; height: ${v.topbarH}px;
    ...
    .po-topbar-left  { flex: 1; display: flex; align-items: center; padding-left: 4px; }
    .po-topbar-center {
    ...
    .po-league-logo   { height: ${v.leagueLogoH}px; width: auto; object-fit: contain; filter: drop-shadow(0 0 8px rgba(255,215,0,.5)); }
    .po-empty-logo    { height: ${v.emptyLogoH}px; width: auto; object-fit: contain; filter: drop-shadow(0 0 18px rgba(255,215,0,.35)); margin-bottom: .5rem; }
    .po-round-label   { font-family: 'Press Start 2P', monospace; font-size: .68rem; color: #FF8C00; letter-spacing: 2px; text-shadow: 0 0 8px rgba(255,140,0,.5); }
    .po-sep-dot       { font-family: 'VT323', monospace; font-size: 1.4rem; color: rgba(255,255,255,.2); }
    .po-game-label    { font-family: 'Barlow Condensed', sans-serif; font-weight: 700; font-size: 1.05rem; letter-spacing: 3px; color: rgba(255,255,255,.65); }
    .po-series-status { font-family: 'Barlow Condensed', sans-serif; font-weight: 800; font-size: 1.1rem; letter-spacing: 2px; color: #87CEEB; text-shadow: 0 0 10px rgba(135,206,235,.4); }
    .po-live-dot {
    ...
    /* ── SERIES HERO BAR ── */
    .po-series-bar {
      position: absolute; top: ${v.topbarH}px;
      left: ${v.sidePanelW + 2}px; right: ${v.sidePanelW + 2}px;
      height: 96px;
    ...
    .po-hero-half         { flex: 1; display: flex; align-items: center; padding: 0 16px; }
    ...
    .po-hero-divider      { width: 2px; flex-shrink: 0; display: flex; align-items: stretch; }
    .po-hero-divider-line {
      width: 2px; flex: 1;
      background: linear-gradient(180deg,
        transparent 0%, rgba(255,215,0,.15) 15%, rgba(255,215,0,.55) 40%,
        rgba(255,255,255,.8) 50%, rgba(255,215,0,.55) 60%,
        rgba(255,215,0,.15) 85%, transparent 100%);
      box-shadow: 0 0 10px rgba(255,215,0,.45), 0 0 22px rgba(255,215,0,.15);
    }
    .po-hero-seed {
      font-family: 'Press Start 2P', monospace; font-size: ${v.heroSeedFontSize};
      color: #FFD700; background: rgba(255,215,0,.08);
      border: 1px solid rgba(255,215,0,.35); border-radius: 4px;
      padding: .22rem .5rem; line-height: 1; white-space: nowrap;
      text-shadow: 0 0 10px rgba(255,215,0,.6);
      flex-shrink: 0; min-width: 3rem; text-align: center;
    }
    .po-hero-logo {
      width: ${v.heroLogoSize}px; height: ${v.heroLogoSize}px; object-fit: contain;
      filter: drop-shadow(0 0 12px rgba(255,215,0,.25)); flex-shrink: 0;
    }
    .po-hero-wins {
      font-family: 'Barlow Condensed', sans-serif; font-weight: 800;
      font-size: ${v.heroWinsFontSize}; line-height: 1; color: #FFD700;
      text-shadow: 0 0 28px rgba(255,215,0,.45), 0 2px 8px rgba(0,0,0,.9);
      flex-shrink: 0; min-width: 3.4rem; text-align: center;
    }
    .po-dots { display: flex; align-items: center; gap: 7px; flex-shrink: 0; }
    .po-dot  { width: 13px; height: 13px; border-radius: 50%; transition: all .3s ease; }
    .po-dot.filled.left  { background: radial-gradient(circle, #87CEEB, #4a9fc4); box-shadow: 0 0 9px rgba(135,206,235,.8), 0 0 18px rgba(135,206,235,.3); }
    .po-dot.filled.right { background: radial-gradient(circle, #FF8C00, #cc6600); box-shadow: 0 0 9px rgba(255,140,0,.8), 0 0 18px rgba(255,140,0,.3); }
    .po-dot.empty { background: transparent; border: 2px solid rgba(255,255,255,.18); }

    /* ── SIDE PANELS ── */
    .po-side-left, .po-side-right {
      position: absolute; top: ${v.topbarH}px; bottom: ${v.scrollerH}px;
      width: ${v.sidePanelW}px;
    ...
    .po-side-header {
      display: flex; align-items: center; gap: 8px; padding: .45rem .55rem;
      background: linear-gradient(90deg, rgba(255,140,0,.18), transparent);
      border-bottom: 1px solid rgba(255,215,0,.18);
      position: sticky; top: 0; z-index: 2;
    }
    .po-side-logo { width: ${v.sidePanelLogoSize}px; height: ${v.sidePanelLogoSize}px; object-fit: contain; filter: drop-shadow(0 0 4px rgba(255,215,0,.28)); }
    .po-side-code { font-family: 'Press Start 2P', monospace; font-size: ${v.sideCodeFontSize}; color: #FFF; letter-spacing: 1px; }
    .po-section-label {
      font-family: 'Press Start 2P', monospace; font-size: .48rem;
      color: #FF8C00; letter-spacing: 2px; padding: .3rem .55rem .16rem;
    ...
    .po-td { font-family: 'VT323', monospace; font-size: 1.28rem; color: #C8D8E8; padding: .08rem .32rem; text-align: center; line-height: 1.25; }
    ...
    .po-no-data { font-family: 'Press Start 2P', monospace; font-size: .42rem; color: rgba(255,255,255,.13); text-align: center; padding: 2rem .5rem; }
    .po-side-divider { height: 1px; background: linear-gradient(90deg, transparent, rgba(255,215,0,.3), transparent); margin: .4rem .55rem; }
    .po-stats-grid { display: flex; flex-direction: column; gap: 0; padding: .1rem 0 .5rem; }
    .po-stat-row {
      display: grid; grid-template-columns: 2.6rem 1fr auto;
      align-items: center; padding: .22rem .55rem;
      border-bottom: 1px solid rgba(255,255,255,.04); gap: .3rem;
    }
    .po-stat-row:nth-child(even) { background: rgba(255,255,255,.017); }
    .po-stat-label { font-family: 'Press Start 2P', monospace; font-size: .40rem; color: rgba(255,255,255,.5); letter-spacing: 1px; text-align: left; }
    .po-stat-val   { font-family: 'VT323', monospace; font-size: 1.42rem; color: #FFFFFF; text-align: left; line-height: 1; }
    ...
    .po-stat-sub { font-family: 'Barlow Condensed', sans-serif; font-size: .78rem; color: rgba(255,255,255,.38); text-align: right; white-space: nowrap; }

    /* ── BOTTOM SCROLLER ── */
    .po-scroller {
      position: absolute; bottom: 0; left: 0; right: 0; height: ${v.scrollerH}px;
      background: linear-gradient(90deg,
        rgba(2,1,8,.99) 0%, rgba(6,3,18,.99) 15%,
        rgba(6,3,18,.99) 85%, rgba(2,1,8,.99) 100%);
      border-top: 2px solid rgba(255,215,0,.28); overflow: hidden;
    }
    .po-scroller-inner { display: flex; align-items: center; height: 100%; white-space: nowrap; will-change: transform; }
    .sc-item {
      display: inline-flex; align-items: center; gap: 7px; padding: 0 28px;
      font-family: 'Barlow Condensed', sans-serif;
    ...
    .sc-pill  { font-family: 'Press Start 2P', monospace; font-size: .56rem; color: #FF8C00; background: rgba(255,140,0,.14); border: 1px solid rgba(255,140,0,.3); border-radius: 2px; padding: .1rem .3rem; }
    .sc-team  { font-family: 'Barlow Condensed', sans-serif; font-weight: 800; font-size: 1.05rem; color: rgba(255,255,255,.65); letter-spacing: 1px; }
    .sc-team.win { color: #FFD700; text-shadow: 0 0 7px rgba(255,215,0,.4); }
    .sc-score { font-family: 'Barlow Condensed', sans-serif; font-weight: 800; font-size: 1.25rem; color: #E8F0F8; }
    ...
    /* ── EMPTY / LOADING ── */
    .po-empty { position: absolute; inset: 0; background: linear-gradient(170deg, rgba(4,2,14,.97), rgba(8,6,22,.97)); display: flex; align-items: center; justify-content: center; }
    .po-empty-inner { text-align: center; display: flex; flex-direction: column; align-items: center; gap: .8rem; }
    .po-empty-title { font-family: 'Press Start 2P', monospace; font-size: 1.2rem; color: rgba(255,215,0,.4); letter-spacing: 2px; }
    .po-empty-sub   { font-family: 'VT323', monospace; font-size: 1.4rem; color: rgba(255,255,255,.25); }
    .key            { font-family: 'Press Start 2P', monospace; font-size: 1.6rem; color: #FF8C00; }
    .po-loading     { font-family: 'Press Start 2P', monospace; font-size: 1rem; color: #FFD700; animation: bpulse 1s ease-in-out infinite; }
    @keyframes bpulse { 0%,100% { filter: drop-shadow(0 0 4px #FF8C00); opacity: 1; } 50% { filter: drop-shadow(0 0 12px #FFD700); opacity: .6; } }
    ...
    .csel-list {
      ...
      max-height: 200px; overflow-y: auto; box-shadow: 0 8px 24px rgba(0,0,0,.8);
    }
