...
export default function StreamOverlayPlayoff() {
  ...
  const h2hCacheRef = useRef(null); // { key: 'tA-tB', h2h } — only refetched when matchup changes
  const lastFingerprintRef = useRef(null); // tiny snapshot of bracket scores/seeds — skips full poll when unchanged
  ...
  const loadMatchup = useCallback(async (tA, tB, lg, round, series, isPoll = false) => {
    ...
    // Snapshot scores/seeds so the poll loop can skip a full refresh next time
    ...
    // ── H2H data — fetched ONCE per matchup, cached across polls ────────
    ...
    if (isPoll && h2hCacheRef.current?.key === matchupKey) {
      ...
      // Scoped to this matchup's two coaches only, not the whole season
