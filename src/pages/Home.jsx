...
function GameCard({ game, index }) {
  ...
  return (
    <div className="gc" style={{ animationDelay: `${index * 0.05}s` }}>
      ...
      <div className="gc-body">
        <div className={`gc-slot ${homeWin ? 'gc-win' : ''}`}>
          <img
            src={`/assets/teamLogos/${game.home}.png`}
            alt={game.home} className="gc-logo"
            onError={e => { e.currentTarget.style.display='none'; e.currentTarget.nextElementSibling.style.display='flex'; }}
          />
          <div className="gc-logo-fb">{(game.home||'').slice(0,3)}</div>
          <span className="gc-score">{game.score_home ?? '—'}</span>
        </div>
        <div className="gc-sep">–</div>
        <div className={`gc-slot ${awayWin ? 'gc-win' : ''}`}>
          <span className="gc-score">{game.score_away ?? '—'}</span>
          <img
            src={`/assets/teamLogos/${game.away}.png`}
            alt={game.away} className="gc-logo"
            onError={e => { e.currentTarget.style.display='none'; e.currentTarget.nextElementSibling.style.display='flex'; }}
          />
    ...
    (async () => {
      ...
      if (recentLgCodes.length > 0) {
        const { data: gamesData } = await supabase
          .from('games')
          .select('lg, game, home, away, score_home, score_away, ot, mode')
          .in('lg', recentLgCodes)
        ...
        <div className="cg-col">
          ...
          <section className="panel">
            ...
            <div className="streaks">
              <div className="coming-soon streaks-cs">
                <div className="streak-preview">
                  {['W','W','W','W','L'].map((r,i) => (
                    <span key={i} className={`sp-dot ${r==='W'?'sp-w':'sp-l'}`}>{r}</span>
                  ))}
                  <span className="sp-fade">…</span>
                </div>
                <div className="cs-label">WIN / LOSS STREAKS COMING SOON</div>
              </div>
            </div>
          </section>
        </div>
        ...
        <div className="ticker-track">
          {tickerList.length > 0 ? (
            <div className="ticker-reel">
              {tickerList.map((g, i) => (
                <span key={i} className="ti">
                  <img src={`/assets/teamLogos/${g.home}.png`} alt="" className="ti-logo"
                    onError={e => { e.currentTarget.style.display='none'; }} />
                  <span className="ti-team">{g.home}</span>
                  <span className={`ti-score ${Number(g.score_home) > Number(g.score_away) ? 'ti-w':''}`}>{g.score_home}</span>
                  <span className="ti-sep">–</span>
                  <span className={`ti-score ${Number(g.score_away) > Number(g.score_home) ? 'ti-w':''}`}>{g.score_away}</span>
                  <span className="ti-team">{g.away}</span>
                  <img src={`/assets/teamLogos/${g.away}.png`} alt="" className="ti-logo"
                    onError={e => { e.currentTarget.style.display='none'; }} />
            ...
            <span className="ticker-placeholder">
              LEAGUE NEWS &amp; RESULTS WILL APPEAR HERE &nbsp;◆&nbsp; STAY TUNED &nbsp;◆&nbsp;
              LEAGUE NEWS &amp; RESULTS WILL APPEAR HERE &nbsp;◆&nbsp; STAY TUNED &nbsp;◆&nbsp;
            </span>
          )}
        </div>
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .gs-label {
          ...
          color: rgba(255,140,0,.55); letter-spacing: 3px;
        }
        .gs-cards {
        ...
        .gc-slot { display: flex; flex-direction: column; align-items: center; gap: .18rem; flex: 1; }
        .gc-win .gc-score { color: #FFD700 !important; text-shadow: 0 0 8px rgba(255,215,0,.55); }
        .gc-logo { width: 26px; height: 26px; object-fit: contain; filter: drop-shadow(0 0 3px rgba(135,206,235,.2)); }
        .gc-logo-fb {
          ...
          border-radius: 4px; font-family: 'Press Start 2P', monospace; font-size: .26rem; color: #87CEEB;
        }
        .gc-score { font-family: 'VT323', monospace; font-size: 1.35rem; color: rgba(255,255,255,.45); line-height: 1; }
        .gc-sep { font-family: 'VT323', monospace; font-size: .9rem; color: rgba(255,140,0,.3); padding: 0 .05rem; }
        .gc-logo-ph { width: 26px; height: 26px; background: rgba(255,255,255,.06); border-radius: 4px; }
        .gc-score-ph { width: 18px; height: 14px; background: rgba(255,255,255,.06); border-radius: 3px; }
        ...
        .cb-label {
          ...
          color: rgba(255,140,0,.4); letter-spacing: 3px;
        }
