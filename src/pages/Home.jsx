...
// Raw ms diff fails because "2026-02-24T09:00:00" without Z is ambiguous —
// JS parses it as local time on some engines and UTC on others.
// Forcing a local-date comparison avoids the off-by-one-day bug entirely.
function daysUntil(iso) {
  if (!iso) return null;
  // Ensure we parse as a real moment (append Z if no tz info present)
  const normalized = /[Zz]|[+-]\d{2}:?\d{2}$/.test(iso) ? iso : iso + 'Z';
  const eventMs = new Date(normalized).getTime();
  if (isNaN(eventMs)) return null;
  if (eventMs < Date.now()) return null;

  // Compare calendar days in local timezone
  const now   = new Date();
  const event = new Date(eventMs);
  const todayMidnight = new Date(now.getFullYear(),  now.getMonth(),   now.getDate()).getTime();
  const evtMidnight   = new Date(event.getFullYear(), event.getMonth(), event.getDate()).getTime();
  const days = Math.round((evtMidnight - todayMidnight) / 86400000);
...
export default function Home() {
  ...
  // ── Discord events (league-agnostic) ──────────────────────────────────────
  useEffect(() => {
    const refresh = async () => {
      setEvtLoading(true);
    ...
    <div className="hp">
      ...
      <div className="cg">
        ...
        <div className="cg-col">
          ...
          {/* ━━━ MEDIA CLUSTER: Discord Events + Twitch ━━━ */}
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .lc::before {
          ...
          background: radial-gradient(ellipse 80% 60% at 50% 0%,
            color-mix(in srgb, var(--lc) 8%, transparent), transparent 70%);
          pointer-events: none;
        }
        .lc:has(.lc-n) {
          box-shadow: 0 0 28px color-mix(in srgb, var(--lc) 10%, transparent),
                      inset 0 1px 0 color-mix(in srgb, var(--lc) 15%, transparent);
        }
        ...
        /* ── Media cluster: Discord + Twitch stacked with shared visual treatment ── */
        .media-cluster {
          display: flex;
          flex-direction: column;
          gap: 0;
          border: 1.5px solid rgba(88,101,242,.18);
          border-radius: 10px;
          overflow: hidden;
          /* Separate the two panels with an inner divider line */
        }
        /* Remove individual panel borders inside media-cluster so they share the outer border */
        .media-cluster > .panel {
          border: none;
          border-radius: 0;
        ...
        /* TwitchLiveWidget is a section.twg-panel inside media-cluster */
        .media-cluster > .twg-panel {
          border: none;
          border-radius: 0;
          border-top: 1px solid rgba(0,255,100,.08);
        }
        .media-cluster > .twg-panel-live {
          border-top-color: rgba(0,255,100,.22);
          box-shadow: none; /* suppress external glow since it's inside a container */
        }
        ...
        .event-row {
          ...
          padding: .38rem .75rem; text-decoration: none;
        ...
        .ev-cal {
          ...
          border-radius: 5px; padding: .22rem .45rem; min-width: 36px; flex-shrink: 0;
        }
        ...
        .ev-live-badge {
          ...
          background:rgba(0,255,100,.12); border:1px solid rgba(0,255,100,.35); color:#00FF64;
          padding:.12rem .34rem; border-radius:3px; animation:blink 1.4s ease-in-out infinite;
        }
        ...
        .streak-section-lbl {
          ...
          letter-spacing: 2px; margin-bottom: .35rem; padding: .15rem 0;
        }
        ...
        .ticker-tag {
          ...
          font-family: 'Press Start 2P', monospace; font-size: .44rem;
          color: #000; letter-spacing: 3px; white-space: nowrap;
          border-right: 2px solid #FFD700; flex-shrink: 0;
        }
        ...
        .ticker-placeholder {
          font-family:'VT323',monospace; font-size:1.05rem; color:rgba(255,255,255,.15);
          letter-spacing:4px; animation:scroll 22s linear infinite; white-space:nowrap; will-change:transform;
        }
