...
async function fetchGazetteEdition({
  ...
  // Null-safe traits map: manager.id → traits object
  const traitsMap = managers.reduce((acc, m) => {
    if (m?.id && m.manager_traits) {
      acc[m.id] =
  ...
  try {
    const { data: cached } = await supabase
      .from('gazette_cache')
      .select('data, date')
      .eq('league', isPlayoffActive ? `${leagueLabel}_playoff` : leagueLabel)
  ...
  // ── Traits lines: only include teams whose manager has traits
  const traitsLines = (teams || [])
    .map((t) => {
      const managerId = t.manager_id;
      const traits = traitsMap[managerId];
      if (!traits) return null; // skip if no traits

      const teamName = tn(t.abr)?.full || t.abr;
      const coachName = t.coach || 'Unknown';
      return `${teamName} (${t.abr}) — coached by ${coachName}, who is a ${traits.media}, ${traits.style} strategist with a ${traits.philosophy} philosophy and a ${traits.temperament} temperament.`;
    })
    .filter(Boolean)
    .join('\n');
  ...
  // ── Build the rest of your prompt and call AI...
  const prompt = `You are the sharp-tongued editor of ${leagueLabel} MAGAZINE for season ${season}.
Today's story angle: "${isPlayoffActive ? 'PLAYOFF ACTION' : 'idle'}".
━━━━━━━━━━━━━━━━━━━━━━━━━━━
  TEAM TRAITS
${traitsLines || '(no traits yet)'}
`;
  ...
  try {
    await supabase.from('gazette_cache').upsert({
      league: isPlayoffActive ? `${leagueLabel}_playoff` : leagueLabel,
      date: today,
      data,
    });
